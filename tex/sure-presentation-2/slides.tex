...
\begin{frame}{Hypotheses Galore}
    ...
    many recent trials in studying the effect of A$\beta$ blockers are failing to have meaningful effects. \cite{kepp2023amyloid}.
...
\begin{frame}
    Observing the model when a constant treatment for A$\beta$, Ca, and $\tau$, it is suggested treatments on an interuption of calcium is not as relevant as initially assumed; rather, tau is the most important treatment avenue.
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.8\textwidth]{img/alzgraph2.pdf}
        \caption{Constant treatments on A $\beta$, Ca, and $\tau$.}
    \end{figure}
\end{frame}
