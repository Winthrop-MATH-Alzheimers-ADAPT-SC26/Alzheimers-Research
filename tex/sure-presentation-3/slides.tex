...
\begin{frame}{On Invariance and Boundedness}
    ...
    The system is also \textit{positively invariant} in the non-negative orthant. This is shown by 
    \begin{equation*}
        \left. \frac{dA_{\beta}}{dt} \right|_{A_{\beta} = 0}, \left. \frac{dCa}{dt} \right|_{Ca = 0}, \left. \frac{d\tau}{dt} \right|_{\tau = 0}, \left. \frac{dN}{dt} \right|_{N = 0}, \left. \frac{dC}{dt} \right|_{C = 0} \geq 0.
    \end{equation*}
    ...
    Let $r_n = k_n + u_n$. The system can also be shown to be \textit{bounded} by
    \begin{alignat*}{4}
        &\frac{dA_\beta}{dt} &&= a_{1} + \textcolor{red}{a_{2} \frac{Ca}{Ca + \sigma}} - r_{1} A_{\beta} &&\leq a_{1} + \textcolor{red}{a_{2}} - r_{1} A_{\beta} &&<0 \, \text{ if } \, A_{\beta} > \frac{a_{1} + a_{2}}{r_{1}}, \\
        &\frac{dCa}{dt} &&= b_{1} + \textcolor{red}{b_{2} A_{\beta}} - r_{2} Ca &&\leq b_{1} + \textcolor{red}{b_{2} M} - r_{2} Ca &&< 0 \, \text{ if } \, Ca > \frac{b_{1} + b_{2} M}{r_{2}}.
    \end{alignat*}
    Since $A _{\beta}$ and $Ca$ feed into the other compartments, it follows that each subsequent compartment is also bounded.
...
\begin{frame}{Discussion of Proposed Model}
    ...
    \begin{eqnarray*}
        \frac{dA_{\beta}}{dt} &=& a_{1} + a_{2} \frac{Ca}{Ca + \sigma} - r_{1} A_{\beta} \\
        \frac{dCa}{dt} &=& b_{1} + b_{2} A_{\beta} - r_{2} Ca
    \end{eqnarray*}
    ...
    For showing the number of equilibria, solve $dA _{\beta}/dt=0$ in terms of $A _{\beta}$. Substituting this into $dCa/dt$ and simplifying (using CAS), a quadratic is obtained of the form 
        ...
        Using \textit{Descartes' Rule of Signs}, there is one positive real root for the quadratic, and hence a unique positive real equilibrium ($A_{\beta}^{*}, Ca^{*}$) exists. 
    }
    ...
    \end{eqnarray*}
    Since a globally stable equilibrium has been determined for the uncoupled system, it follows that $\tau$ will also tend toward some equilibrium that is globally stable. This provides a similar cascade to $N$ and $C$, and thus there exists a unique globally asymptotically stable equilibrium for the proposed model.}
\end{frame}
...
\begin{frame}{Optimal Control}
    Recall that equations have the form:
    \begin{align*}
        \frac{dA_{\beta}}{dt} &= a_{1} + a_{2} \frac{Ca}{Ca+\sigma} - k_{1} A_{\beta} - \textcolor{red}{u_{1} A_{\beta}}.
    \end{align*}
